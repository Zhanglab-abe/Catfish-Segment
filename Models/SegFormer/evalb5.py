...
import wandb
import evaluate
wandb.init(project="segb5", entity="msu-zhanglab")
...
for root, dirs, files in os.walk(f"{sys.path[0]}/data/test/data_raw/images_raw/"):
    ...
    break
for root, dirs, files in os.walk(f"{sys.path[0]}/data/test/data_raw/annotations/"):
    # print(root, dirs, files)
    files.sort()
    for file in files:
...
my_pretrained_model_name = "/home/agricoptics/Desktop/CatFish/segformer/segformer-b5-mydataSet/checkpoint-7200"
...
# trainer = Trainer(model=model, data_collator=lambda x: x, compute_metrics=compute_metrics)
...
trainer = Trainer(model=model,
                  args = training_args, 
                    compute_metrics=compute_metrics)
# predictions = trainer.evaluate(test_dataset=test_ds).predictions
# test_metrics = compute_metrics((predictions, predictions.label_ids))
eval_results = trainer.evaluate(test_ds)
out_file = open("result/segb5evaltest.json", "w") 
